...
def _test_gateways(gateway, url="https://doi.org/10.1038/171737a0", **kwargs):
    ...
    with get_driver() as driver:
        useless_title, useless_texts = useless_crawler.get_contents(url=url, driver=driver)

    # Crawler with gateway.
    gateway_crawler = journals.get(identifier=journal_type, gateway=gateway)
    with get_driver() as driver:
        gateway_title, gateway_texts = gateway_crawler.get_contents(url=url, driver=driver)
        
    assert useless_title == gateway_title
    assert len(useless_texts) < len(gateway_texts)
