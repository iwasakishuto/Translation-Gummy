...
def _test_translators(db, identifier, **kwargs):
    with get_driver() as driver:
        ...
        ja_simple_sentence    = translator.en2ja(query=db.simple_sentence)
        ja_multiple_sentences = translator.en2ja(query=db.multiple_sentences)
        ja_SteveJobsSpeech    = translator.en2ja(query=db.SteveJobsSpeech)
